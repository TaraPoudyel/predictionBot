...
module.exports.run =  async (bot, msg, arg, con, week) =>
{
  ...
  else
  {
    ...
    con.query(`SELECT * FROM predictions WHERE user_id = '${msg.author.id}' AND id = '${match}' AND week = '${week}'`, (err, rows) => {
      if(rows.length == 0)
      {
        con.query(`SELECT * FROM matches WHERE id = '${match}'`, (err, rows) => {
          if(err) throw err;
          if(rows.length == 0)
          {
          ...
          {
            let sql;
            sql = `INSERT INTO predictions (user_id, home, away, fixture_id, home_name, away_name, id) VALUES ('${msg.author.id}', '${home}', '${away}', '${fixture}', '${rows[0].home_name}', '${rows[0].away_name}', '${match}')`
